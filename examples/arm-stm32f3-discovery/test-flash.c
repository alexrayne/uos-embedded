...
ARRAY (task, 1000);
stm32f3_spim_t spi;
stm32f3_gpio_t pin_miso;
stm32f3_gpio_t pin_mosi;
stm32f3_gpio_t pin_clk;
stm32f3_gpio_t pin_cs;
timer_t timer;
...
void uos_init (void)
{
	...
	stm32f3_gpio_init(&pin_cs,   GPIO_PORT_A, 4, GPIO_FLAGS_OUTPUT | GPIO_FLAGS_HIGH_SPEED);
	gpio_set_val(to_gpioif(&pin_cs), 1);
	stm32f3_gpio_init(&pin_clk,  GPIO_PORT_A, 5, GPIO_FLAGS_ALT_FUNC(5) | GPIO_FLAGS_HIGH_SPEED);
	stm32f3_gpio_init(&pin_miso, GPIO_PORT_A, 6, GPIO_FLAGS_ALT_FUNC(5) | GPIO_FLAGS_HIGH_SPEED);
	stm32f3_gpio_init(&pin_mosi, GPIO_PORT_A, 7, GPIO_FLAGS_ALT_FUNC(5) | GPIO_FLAGS_HIGH_SPEED);
	
	timer_init(&timer, KHZ, 1);
	
    stm32f3_spi_init(&spi, 1, spi_cs_control);
