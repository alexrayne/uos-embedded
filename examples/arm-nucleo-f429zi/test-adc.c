...
// Используем 3 АЦП в режиме interleaved.
// Начинаем запись трассы в буфер в памяти по превышению уровня на входе
// заданного значения, используя аналоговый сторож.
//
...
#define ADC_DATA_WINDOW     (16 * 1024)
...
uint16_t adc_data[ADC_DATA_WINDOW];
...
void adc_init()
{
    ...
    asm volatile ("dmb");
    DMA2_STR(0)->PAR = (unsigned)&ADC1->DR;
    DMA2_STR(0)->M0AR = (unsigned)adc_data;
    DMA2_STR(0)->NDTR = ADC_DATA_WINDOW / 4;
    DMA2_STR(0)->CR = DMA_CHSEL(0) | DMA_MSIZE_16 | DMA_PSIZE_16 | DMA_MINC | 
    ...
    ADC1->CR2 = ADC_CONT | ADC_ADON | ADC_EOCS | ADC_DMA | ADC_DDS;
    ADC1->SQR1 = ADC_L(1);
    ...
    // Independent mode
    ADC_COM->CCR = ADC_ADCPRE_DIV2 | ADC_MULTI_INDEP | ADC_DMA_MODE1;
