...
static inline unsigned get_apb_div(stm32f4_spim_t *spi)
{
    if (spi->port == 2 || spi->port == 3)
        return APB1_DIV;
    else
        return APB2_DIV;
}
...
static inline int
init_hw(stm32f4_spim_t *spi, unsigned freq, unsigned bits_per_word, unsigned mode)
{
    SPI_t *reg = spi->reg;
    ...
    if (freq != spi->last_freq) {
        ...
        unsigned fPCLK = KHZ * 1000 / get_apb_div(spi);
...
{
    stm32f4_spim_t      *spi = (stm32f4_spim_t *) spimif;
    SPI_t               *reg = spi->reg;
    uint8_t             *rxp_8bit;
    uint8_t             *txp_8bit;
    uint16_t            *rxp_16bit;
...
static int trx(spimif_t *spimif, spi_message_t *msg)
{
    ...
    res = init_hw((stm32f4_spim_t *)spimif, msg->freq, bits_per_word, mode);
    if (res != SPI_ERR_OK) {
...
int stm32f4_spi_init(stm32f4_spim_t *spi, unsigned port, spi_cs_control_func csc)
{
    memset(spi, 0, sizeof(stm32f4_spim_t));
    ...
    if (port == 1) {
        ...
        RCC->APB2LPENR |= RCC_SPI4LPEN;
        spi->reg = SPI4;
    } else if (port == 5) {
        RCC->APB2ENR |= RCC_SPI5EN;
        RCC->APB2LPENR |= RCC_SPI5LPEN;
        spi->reg = SPI5;
    } else if (port == 6) {
        RCC->APB2ENR |= RCC_SPI6EN;
        RCC->APB2LPENR |= RCC_SPI6LPEN;
        spi->reg = SPI6;
    } else {
